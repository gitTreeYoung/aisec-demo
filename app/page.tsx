...
export default function ThreatSimulation() {
  ...
  const getKeywordHighlight = (text: string) => {
    const redKeywords = ["钓鱼邮件", "资金安全", "恶意链接", "窃取"]
    const orangeKeywords = ["verify", "login"]
    const greenKeywords = ["异常活动", "账户"]

    for (const keyword of redKeywords) {
      if (text.includes(keyword)) {
        return "bg-[#fef2f2] text-[#dc2626] px-1 rounded"
      }
    }

    for (const keyword of orangeKeywords) {
      if (text.toLowerCase().includes(keyword.toLowerCase())) {
        return "bg-[#fff7ed] text-[#ea580c] px-1 rounded"
      }
    }

    for (const keyword of greenKeywords) {
      if (text.includes(keyword)) {
        return "bg-[#f0fdf4] text-[#16a34a] px-1 rounded"
      }
    }
